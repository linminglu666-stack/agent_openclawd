...
def load_module(name):
    spec = importlib.util.spec_from_file_location(name, os.path.join(module_dir, f"{name}.py"))
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module

types_mod = load_module("types")
calculator_mod = load_module("calculator")
monitor_mod = load_module("monitor")
sweeper_mod = load_module("sweeper")
attribution_mod = load_module("attribution")
adaptive_threshold_mod = load_module("adaptive_threshold")
persistence_mod = load_module("persistence")
engine_mod = load_module("engine")

EntropyLevel = types_mod.EntropyLevel
EntropyCategory = types_mod.EntropyCategory
AlertSeverity = types_mod.AlertSeverity
SweepPriority = types_mod.SweepPriority
SweepStatus = types_mod.SweepStatus
EntropyThreshold = types_mod.EntropyThreshold
EntropySample = types_mod.EntropySample
EntropyAlert = types_mod.EntropyAlert
SweepAction = types_mod.SweepAction
TrendAnalysis = types_mod.TrendAnalysis
AttributionResult = types_mod.AttributionResult
AdaptiveThreshold = types_mod.AdaptiveThreshold
EntropyCalculator = calculator_mod.EntropyCalculator
MetricDefinition = calculator_mod.MetricDefinition
exponential_decay = calculator_mod.exponential_decay
weighted_entropy = calculator_mod.weighted_entropy
EntropyMonitor = monitor_mod.EntropyMonitor
AlertRule = monitor_mod.AlertRule
EntropySweeper = sweeper_mod.EntropySweeper
SweepStrategy = sweeper_mod.SweepStrategy
EntropyAttributor = attribution_mod.EntropyAttributor
AdaptiveThresholdManager = adaptive_threshold_mod.AdaptiveThresholdManager
EntropyPersistence = persistence_mod.EntropyPersistence
EntropyEngine = engine_mod.EntropyEngine
EntropyEngineConfig = engine_mod.EntropyEngineConfig
...
class TestMonitor(unittest.TestCase):
    ...
    def test_analyze_trend_with_samples(self):
        for i in range(20):
            self.calculator.record_sample(
                "inbox_stale", 0.3 + i * 0.01, "test_source"
            )
